...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and werkzeug.security.check_password_hash(user.password, password):
            ...
            flash("Sorry, that password is not correct.")
            return redirect(url_for("login"))
...
def show_post(post_id):
    ...
    if current_user.is_authenticated:
        ...
        return render_template("post.html", post=requested_post, id=user_id, form=form)
    else:
        if form.validate_on_submit():
            flash("To leave a comment make sure to log in first.")
            return redirect(url_for("login"))
